...
export const likesRouter = router({
  getLikes: publicProcedure
    ...
    .query(async ({ ctx: { db }, input: { resourceId, authorId } }) => {
      const countList = await db
        ...
        .from(likes)
        .where(
          and(eq(likes.resourceId, resourceId), eq(likes.authorId, authorId)),
        );
  ...
  like: protectedProcedure
    .input(SelectLikeSchema)
    .mutation(
      async ({ ctx: { db, userId }, input: { authorId, resourceId } }) => {
  ...
  unlike: protectedProcedure
    .input(SelectLikeSchema)
    .mutation(
      async ({ ctx: { db, userId }, input: { resourceId, authorId } }) => {
        await db
          .delete(likes)
          .where(
            and(
