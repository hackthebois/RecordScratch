...
export const profilesRouter = router({
  ...
  getSignedURL: protectedProcedure
    ...
    .mutation(async ({ ctx: { userId, r2, env }, input: { type, size } }) => {
      const res = await r2.sign(`${env.R2_ENDPOINT}/profile-images/${userId}`, {
        method: "PUT",
        headers: {
          "Content-Type": type,
          "Content-Length": `${size}`,
  ...
  search: publicProcedure
    ...
    .query(
      async ({ ctx: { db }, input: { query, limit = 10, cursor = 0 } }) => {
